...
def main():
    ...
    # 启动代理脚本
    agent1_process = start_script('agent1.py')
    agent2_process = start_script('agent2.py')
    agent3_process = start_script('agent3.py')

    # 确保所有代理脚本都已启动
    time.sleep(2)  # 适当等待确保所有代理都已启动

    # 启动拍卖协调者脚本
    auction_process = start_script('coordinator.py')
